...
<div {{ $attributes->merge(['class' => 'relative flex justify-center ' . $classes]) }}>
    <img src="{{ Vite::asset('resources/images/logo.svg') }}"
        class="absolute h-full w-auto transition duration-500 ease-in-out opacity-100 dark:opacity-0"
        alt="Logo {{ config('app.name', 'Bubba Camper') }}">

    <img src="{{ Vite::asset('resources/images/logo-dark.svg') }}"
        class="h-full w-auto transition duration-500 ease-in-out opacity-0 dark:opacity-100"
        alt="Logo {{ config('app.name', 'Bubba Camper') }}">
</div>
