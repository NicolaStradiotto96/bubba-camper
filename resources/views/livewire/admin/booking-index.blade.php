                                ...
                                <td class="px-2 py-4 text-xs uppercase">
                                    <div class="flex flex-col items-center gap-1">
                                        ...
                                        <div class="flex justify-between w-36 pt-1">
                                            ...
                                            @elseif (str_starts_with($booking->status, 'cancelled') && $booking->payment_status === 'penalty_paid')
                                                <span class="text-green-600">
                                                    {{ number_format($booking->penalty_amount - $booking->down_payment, 2, ',', '') }}€
                                                </span>
                                            @elseif (str_starts_with($booking->status, 'cancelled') &&
                                                    in_array($booking->payment_status, ['penalty_pending', 'penalty_verification', 'paid']))
                                                <span class="text-amber-500 animate-pulse">
                                                    {{ number_format($booking->remaining_penalty, 2, ',', '') }}€
                                                </span>
                                            @elseif (str_starts_with($booking->status, 'cancelled') || $booking->status === 'expired')
                                                <span class="text-gray-400">0,00€</span>
                                            @else
                                        ...
                                        @if (
                                            ...
                                            <div
                                                ...
                                                <span
                                                    class="text-amber-500 animate-pulse">{{ number_format($booking->penalty_amount, 2, ',', '') }}€
                                                </span>
                                            </div>
                                        @elseif (
                                            $booking->status === 'cancelled' &&
                                            ...
                                            <div
                                                ...
                                                <span class="text-gray-400">Penale:</span>
                                                <span
                                                    class="text-green-600">{{ number_format($booking->penalty_amount, 2, ',', '') }}€
                                                </span>
                                            </div>
                                        @endif
                                        ...
                                        {{-- Refund --}}
                                        @if (str_starts_with($booking->status, 'cancelled') &&
                                                $booking->refund_amount > 0 &&
                                                !in_array($booking->payment_status, ['penalty_paid', 'refunded_stripe', 'refunded_manual']))
                                            <div
                                                class="flex justify-between w-36 pt-1 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <td class="px-2 py-4">
                                    <div class="flex flex-col gap-2 justify-center uppercase">
                                        ...
                                        @if (
                                            ...
                                            <button type="button"
                                                onclick="event.stopPropagation(); confirmPayment('{{ $booking->id }}', {{ $booking->status === 'cancelled' ? max(0, $booking->penalty_amount - $booking->down_payment) : $booking->balance_payment }})"
                                        ...
                                        @if (
                                            $booking->status !== 'cancelled' &&
                                                ($booking->payment_status === 'paid' || $booking->payment_status === 'fully_paid'))
                                            <button type="button"
                                                onclick="event.stopPropagation(); confirmRefundAction('{{ $booking->id }}', {{ $booking->down_paid ? $booking->down_payment : 0 }}, 0, {{ $booking->penalty_amount ?? 0 }}, {{ $booking->stripe_payment_id ? 1 : 0 }})"
                        ...
                        <div
                            ...
                            @if (str_starts_with($booking->status, 'cancelled') &&
                                ...
                                <div class="flex justify-between pt-1 border-t border-gray-100 dark:border-gray-700">
                                    <span class="text-gray-400">Rimborso:</span>
                                    <span
                                        class="font-black text-red-600">-{{ number_format($booking->refund_amount, 2, ',', '') }}€
                                    </span>
                                </div>
                            @endif
                        </div>
                ...
                <div
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            {{-- Damage --}}
                            <a :href="b.damage_url" title="Segnala Danno"
                            ...
                            <!-- Edit -->
                            <a :href="b.edit_url" title="Modifica Prenotazione"
                                class="text-amber-600 hover:text-amber-700 dark:hover:text-amber-500 hover:bg-gray-100 dark:hover:bg-gray-700/50 p-1 rounded-xl transition focus:outline-none focus:ring-2 focus:ring-amber-500">
                                <i class="fa-solid fa-pen-to-square text-xl"></i>
                            </a>

                            <!-- Close -->
                            <button @click="closeAll()" type="button" title="Chiudi"
                    ...
                    <div class="text-center">
                        ...
                        <div
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                <template
                                    x-if="b.status && b.status.startsWith('cancelled') && b.payment_status === 'penalty_paid'">
                                    <span class="text-green-600 font-black"
                                        x-text="((b.penaltyRaw - b.down_payment)).toLocaleString('it-IT', {minimumFractionDigits: 2}) + '€'"></span>
                                </template>

                                <template
                                    x-if="b.status && b.status.startsWith('cancelled') && ['penalty_pending', 'penalty_verification', 'paid'].includes(b.payment_status)">
                                    <span class="text-amber-500 animate-pulse font-black"
                                        x-text="b.remainingPenalty"></span>
                                </template>

                                <template
                                    x-if="(b.status && (b.status.startsWith('cancelled') || b.status === 'expired')) && b.payment_status !== 'penalty_paid' && !['penalty_pending', 'penalty_verification', 'paid'].includes(b.payment_status) && b.payment_status !== 'fully_paid'">
                                    <span class="text-gray-400 font-black">0,00€</span>
                                </template>

                                <template
                                    x-if="b.payment_status !== 'fully_paid' && !(b.status && b.status.startsWith('cancelled')) && b.status !== 'expired'">
                            ...
                            <template x-if="b.status === 'cancelled' && b.penaltyRaw > 0">
                                <div
                                    class="flex justify-between text-sm pt-2 border-t border-gray-100 dark:border-gray-700">
                                    <span class="text-gray-400 uppercase">Penale:</span>
                                    <span class="font-black"
                                        :class="['penalty_pending', 'penalty_verification', 'paid'].includes(b.payment_status) ?
                                            'text-amber-500 animate-pulse' : 'text-green-600'"
                            ...
                            {{-- Refund --}}
                            <template
                                x-if="b && b.status && b.status.startsWith('cancelled') && b.refundRaw > 0 && !['penalty_paid', 'refunded_stripe', 'refunded_manual'].includes(b.payment_status)">
                                <div
                                    class="flex justify-between text-sm pt-2 border-t border-gray-100 dark:border-gray-700">
                                    <span class="text-gray-400 uppercase">Rimborso:</span>
                                    <span class="font-black text-red-600" x-text="'-' + b.refund"></span>
                                </div>
                            </template>
                        ...
                        <div x-show="viewingDocs?.status === 'open'" x-trap="viewingDocs"
                            ...
                            <div class="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 p-4 rounded-[2rem] max-w-4xl w-full max-h-[90vh] overflow-y-auto relative z-10"
                                ...
                                <div class="flex justify-between items-center mb-4">
                                    <h4 tabindex="1"
                                        class="text-lg font-bold text-gray-900 dark:text-white uppercase tracking-wider focus:outline-none"
                                        x-text="viewingDocs ? viewingDocs.title : ''"></h4>
                                    <button @click="viewingDocs.status = null" title="Chiudi"
                                        class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 p-1 rounded-xl transition focus:outline-none focus:ring-2 focus:ring-amber-500">
                                        <i class="fa-solid fa-xmark text-xl"></i>
                                    </button>
                                </div>
                                        ...
                                        <template x-if="doc">
                                            <div class="relative min-h-[300px] flex items-center justify-center border-2 rounded-[2rem] overflow-hidden"
                                                ...
                                                <template x-if="doc.toLowerCase().endsWith('.pdf')">
                                                    <a :href="doc"
                                                        class="glightbox w-full h-full flex items-center justify-center bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 p-6 cursor-zoom-in hover:opacity-90 transition">
                                                        <i class="fa-solid fa-file-pdf text-red-500 text-6xl mb-3"></i>
                                                    </a>
                                                </template>

                                                <template x-if="!doc.toLowerCase().endsWith('.pdf')">
                                                    <a :href="doc"
                                                        class="glightbox w-full h-full flex items-center justify-center bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 cursor-zoom-in hover:opacity-90 transition">
                                                        <img :src="doc"
